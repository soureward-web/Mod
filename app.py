...
# ---------------------------------------------------------------
init_db()
st.set_page_config(page_title="نظام إدارة العملاء والمبيعات", layout="wide")
...
with tab_dashboard:
    ...
    if not df.empty:
        ...
        st.info("لا يوجد عملاء بعد. ابدأ من تبويب 'العملاء المحتملون'.")
...
with tab_leads:
    ...
    if clients:
        selected_ids = []
        for c in clients:
            with st.expander(f"{c.name} — {c.status} ({c.category or 'غير محدد'})"):
